...
def extract_pdf(req: SummarizeRequest):
    try:
        texts = extract(req.filename)
        save_text_to_file(texts, req.filename)
...
def summarize_text(req: SummarizeRequest):
    try:
        ...
        summary = summarize_document_with_kmeans_clustering(
            req.texts, llm, embeddings, num_clusters=req.num_clusters
        )
